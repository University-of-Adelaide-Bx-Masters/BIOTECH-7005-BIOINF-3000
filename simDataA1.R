...
#' Covert csv to tsv sed 's/,/\t/g' VMs.csv > VMs.tsv
library(tidyverse)
library(magrittr)
library(randomNames)
...
ids %>%
    lapply(function(x) {
        ...
        hdr <- c(paste(cmt, "This data contains dCt values for", gn, "in", ct[1], "&", ct[2], "cells."), paste(cmt, "Cells are defined as", tr[1], "&", tr[2], "depending on their treatment type."))
        if (blankLine) hdr <- c(hdr, "")
        if (!blankLine) hdr <- c(hdr, paste(cmt, "Data generated by", randomNames(1, ethnicity = sample(c(2,5,6), 1), name.order = "first.last", name.sep = " ")))
