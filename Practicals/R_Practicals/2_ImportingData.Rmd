...
1. Navigate to your `~/data` folder using the `Files` Tab
    - First click the ![](images/home.png) icon then click into `data`
...
2. `transport <- read_csv("~/data/transport.csv")`
...
```{r}
no_header <- read_csv("~/data/no_header.csv", col_names = FALSE, col_types = "-ccnnc")
```
...
```{r}
no_header <- read_csv("~/data/no_header.csv", col_names = FALSE, col_types = "-cnnnc")
```
